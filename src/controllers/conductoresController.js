        ...
        if(reg1==""){
            ...
            if(reg2==""){
                if(data.path != null){
                    ...
                    data.path = foto_name;
                    fs.writeFileSync(`${path.dirname(__dirname)}/uploads/conductores/`+data.path, new Buffer.from(data.imagen64, "base64"), function(err) {});
                }else{
                    data.path = 'null';
                }
...
const obtener_foto_conductor = async function(req, res){
    ...
    fs.stat(`${path.dirname(__dirname)}/uploads/conductores/`+img, function(err){
        if(!err){
            let path_img = `${path.dirname(__dirname)}/uploads/conductores/`+img;
            res.status(200).sendFile(path.resolve(path_img));
        }else{
            let path_img = `${path.dirname(__dirname)}/uploads/default.jpg`;
            res.status(200).sendFile(path.resolve(path_img));
        }
    });
}
...
function eliminar_foto_backend(foto){
    if(fs.existsSync(`${path.dirname(__dirname)}/uploads/conductores/`+foto)){
        fs.unlinkSync(`${path.dirname(__dirname)}/uploads/conductores/`+foto)
    } 
}
