    ...
    try{
        ...
        if(reg1==""){
            if(data.path != null){
                ...
                data.path = foto_name;
                fs.writeFileSync(`${path.dirname(__dirname)}/uploads/autobuses/`+data.path, new Buffer.from(data.imagen64, "base64"), function(err) {});
            }else{
                data.path = 'null';
            }
...
const obtener_foto_autobus = async function(req, res){
    ...
    fs.stat(`${path.dirname(__dirname)}/uploads/autobuses/`+img, function(err){
        if(!err){
            let path_img = `${path.dirname(__dirname)}/uploads/autobuses/`+img;
...
function eliminar_foto_backend(foto){
    if(fs.existsSync(`${path.dirname(__dirname)}/uploads/autobuses/`+foto)){
        fs.unlinkSync(`${path.dirname(__dirname)}/uploads/autobuses/`+foto)
    } 
}
