    ...
    try{
        ...
        if(reg1==""){
            if(data.path != null){
                ...
                data.path = foto_name;
                fs.writeFileSync(`${path.dirname(__dirname)}/uploads/estudiantes/`+data.path, new Buffer.from(data.imagen64, "base64"), function(err) {});
            }else{
                data.path = 'null';
            }
...
const obtener_foto_estudiante = async function(req, res){
    ...
    fs.stat(`${path.dirname(__dirname)}/uploads/estudiantes/`+img, function(err){
        if(!err){
            let path_img = `${path.dirname(__dirname)}/uploads/estudiantes/`+img;
            res.status(200).sendFile(path.resolve(path_img));
        }else{
            let path_img = `${path.dirname(__dirname)}/uploads/default_persona.jpg`;
            res.status(200).sendFile(path.resolve(path_img));
        }
    });
}
